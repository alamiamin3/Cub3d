...
/*   Updated: 2023/08/09 21:16:11 by aalami           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_horizontal_intersect(t_mlx *mlx, int i)
{
	...
	x_intersect = mlx->player.x + ((y_intersect - mlx->player.y ) / tan(mlx->rays[i].ray_angle));
	 ...
	 while ((int)y_intersect / TILE_SIZE >= 0 && (int)y_intersect / TILE_SIZE < mlx->win_h / TILE_SIZE && 
		(int)x_intersect / TILE_SIZE >= 0 && (int)x_intersect / TILE_SIZE < mlx->win_w / TILE_SIZE  && mlx->map[(int)((y_intersect) / TILE_SIZE)][(int)((x_intersect) / TILE_SIZE)] != '1')
	{
		y_intersect += ( y_step);
		x_intersect += (x_step) ;
	}
...
void    get_vertical_intersect(t_mlx *mlx, int i)
{
	...
	y_intersect = (mlx->player.y + ((x_intersect - mlx->player.x) / tan(mlx->rays[i].ray_angle)));
		printf("VERTICAL\n");
		printf("x_b = %f ; y_b = %f\n", round((x_intersect) / TILE_SIZE), round((y_intersect) / TILE_SIZE));
    while ((int)y_intersect / TILE_SIZE >= 0 && (int)y_intersect / TILE_SIZE < mlx->win_h / TILE_SIZE && 
		(int)x_intersect / TILE_SIZE >= 0 && (int)x_intersect / TILE_SIZE < mlx->win_w / TILE_SIZE  && mlx->map[(int)((y_intersect) / TILE_SIZE)][(int)((x_intersect) / TILE_SIZE)] != '1')
	{
		y_intersect += ( y_step) ;
